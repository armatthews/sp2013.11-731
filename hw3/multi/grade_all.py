...
def grade_stream(tm, lm, french_sents, stream):
	def finalize(grades):
		outputs = sorted(grades.iteritems(), key=lambda p: p[1], reverse=True)
		if len(outputs) > 0:
			print "%d\t%s\t%f" % (prev_sent_num, outputs[0][0], outputs[0][1])
		return {}

	grades = {}
	prev_sent_num = None
	for line in stream:
		sent_num, english, _, _ = line.split(' ||| ')
		sent_num = int(sent_num)
		#sent_num = 54 - sent_num

		if sent_num != prev_sent_num:
			grades = finalize(grades)
		e = tuple(english.strip().split())
		f = french_sents[sent_num]
		sys.stderr.write("Grading sentence pair for sentence #%d\r" % sent_num)
...
grade_stream(tm, lm, french_sents, sys.stdin)
